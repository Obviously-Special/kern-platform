...
export class KernWidget {
  ...
  private guideTo(guide: GuideTarget) {
    ...
    // Pulse rings live INSIDE the element, so they travel with it during
    // the smooth scroll — the pulse is the highlight, nothing else.
    const cleanup = pulseElement(el);
    window.setTimeout(() => {
      cleanup();
